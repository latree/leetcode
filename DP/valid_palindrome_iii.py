...
# 第三遍
        # dp[i][j] 表示 从s的i 到 j idx 要成为回文需要删除几个char
        # 递归方程
        # if s[i] == s[j]
        #     dp[i][j] = dp[i + 1][j -1]
        # else:
        ...
        # 知道这个定义以后，那么如果从右往左来开始计算回文。如果s长度为4， 那么就是开始就是s[2:4] 是不是回文就是由s[3:4]推算出来的
        # 进一步想一想在matrix 里的样子就是matrix 的右上角. 所以我们只需要遍历matrix的右上角，或者左下角
#         ['x',  1,   1,   1], 
#         ['x', 'x',  1,   1], 
#         ['x', 'x', 'x',  1], 
#         ['x', 'x', 'x', 'x']        
